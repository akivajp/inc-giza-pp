...
template<class CLS,class MAPPERCLASSTOSTRING>
void HMMTables<CLS,MAPPERCLASSTOSTRING>::writeJumps2(ostream&out) const
{
  for(typename map<AlDeps<CLS>,FlexArray<double> >::const_iterator i=alProb.begin();i!=alProb.end();++i)
    {
      out << i->first.englishSentenceLength << " ";
      for(int a=i->second.low();a<=i->second.high();++a)
	if( i->second[a] )
	  {
	    out << a << ':' << i->second[a] << ';' << ' ';
...
template<class CLS,class MAPPERCLASSTOSTRING>
void HMMTables<CLS,MAPPERCLASSTOSTRING>::readJumps(istream& inf)
{
  massert(PredictionInAlignments == 0);
  massert(alProb.size() == 0);
  if(!inf){
    cerr << "\nERROR: Cannot open HMM alignments file\n";
    return;
  }
  const int j=1, w1=0, w2=0, idx(0);
  string line, sep;
  ...
  while(getline(inf, line)) {
    ...
    ss >> sentLength;
    //cerr << "sentLength " << sentLength << endl;
    AlDeps<CLS> deps(sentLength, idx, j, w1, w2);
...
double HMMTables<CLS,MAPPERCLASSTOSTRING>::getAlProb(int istrich,int k,int sentLength,int J,CLS w1,CLS w2,int j,int iter) const
{
  ...
  int pos=istrich-k;
  switch(PredictionInAlignments)  // is zero
    {
...
void HMMTables<CLS,MAPPERCLASSTOSTRING>::addAlCount(int istrich,int k,int sentLength,int J,CLS w1,CLS w2,int j,double value,double valuePredicted)
{
  ...
  //cerr << "Istrich = " << istrich << "  sentLenght = " << sentLength << endl;
