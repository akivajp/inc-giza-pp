...
string txt1_vocab(""), txt2_vocab("");

int loadVocab() {
  string line, word;
  int ID, cnt;
  if(!txt1_vocab.empty()) {
    ifstream iv(txt1_vocab.c_str());
    while(getline(iv, line)) {
      istrstream ss(line.c_str());
      while(ss >> ID >> word >> cnt) {
        v1[word] = cnt;  // never used
        iid1.push_back(word);
        assert(iid1.size()-1 == ID);
        id1[word]=ID;
      }
    }
  }
  if(!txt2_vocab.empty()) {
    ifstream iv(txt2_vocab.c_str());
    while(getline(iv, line)) {
      istrstream ss(line.c_str());
      while(ss >> ID >> word >> cnt) {
        v2[word] = cnt;  // never used
        iid2.push_back(word);
        assert(iid2.size()-1 == ID);
        id2[word]=ID;
      }
    }
  }
...
int main(int argc,char**argv)
{
  ...
  for(int i=1;i<argc;++i)
    ...
    else if(string(argv[i])=="-txt1-vocab")
      txt1_vocab=argv[++i];
    else if(string(argv[i])=="-txt2-vocab")
      txt2_vocab=argv[++i];
    else
  ...
  if((filenames.size()%2)==1||filenames.size()==0 )
    {
      cerr << argv[0] << " txt1 txt2 [txt3 txt4 -weight w -txt1-vocab v1 -txt2-vocab v2]\n";
  ...
  if(!(txt1_vocab.empty() && txt2_vocab.empty())) loadVocab();
