...
class tmodel{
 ...
 public:
  ...
  //lexmat[i][j] where i = source id and j = target id
...
public:
  ...
  void checkAndAdd(WordIndex e, vector<WordIndex>& f, bool sort) {
    if(sort) std::sort(f.begin(), f.end());
    pair<unsigned int, CPPair> cp;
    vector<pair<unsigned int, CPPair> > cps;
    typename vector<pair<unsigned int, CPPair> >::iterator itr_cps;
    if(e >= lexmat.size()) { // e not in lexmat
      assert((e + 1) < lexmat.size());
      lexmat.resize(e + 1);  // e is 0-indexed
      lexmat[e] = 0;
      for(int i = 0; i < f.size(); ++i) { //   insert all f
        cp.first = f[i];
        cps.push_back(cp);
      }
      lexmat[e] = new vector< pair<unsigned int,CPPair> > (cps);
    }
    else { //e is in lexmat
      for(int i = 0; i < f.size(); ++i) {
        CPPair *p=find(e,f[i]);
        if(!p) { // if f not in lexmat[e] add in right position 
          cp.first = f[i]; 
          if(f[i] > (*lexmat[e]).back().first) 
            (*lexmat[e]).insert((*lexmat[e]).end(), cp);
          else {
            int idx = 0;
            while((*lexmat[e])[idx].first < f[i]) ++idx; 
            (*lexmat[e]).insert((*lexmat[e]).begin() + idx, cp);
          }
        }
      }
    }
  ...
  void normalizeTable(const vcbList&srcl, const vcbList&trgl, const set<WordIndex>* = 0, bool = true);
