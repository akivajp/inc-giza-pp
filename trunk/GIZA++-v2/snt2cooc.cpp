...
void loadCooc(ifstream& in, vector<map<int, int> >& vsi) {
  string line;
  int cnt(0), id1(0), id2(0);
  bool counts(false);
  if(!in) cerr <<"Previous cooc file does not exist.\n";
  getline(in, line);
...
int main(int argc,char **argv)
{
  if( argc!=4&&argc!=5 )
    {
      cerr << "Usage: " << argv[0] << " vcb1 vcb2 snt12 [prevCoocFile]\n";
