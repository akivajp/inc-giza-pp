	...
	for(unsigned int j=0;j<lexmat[i]->size();++j)
	  {
		...
		of << e << ' ' << f << ' ' << x.count << ' ' << x.prob << '\n';
	  }
    }
}
extern float step_k;
extern float step_alpha;
template <class COUNT, class PROB>
COUNT tmodel<COUNT, PROB>::interpolateStepCounts() {
  const float neg_alpha = step_alpha * (-1);
  const float mu_k = pow(step_k + 2, neg_alpha);  
  const float OneMinusMu_k = 1 - mu_k;
  typename map<wordPairIds, COUNT>::const_iterator stpItr = stepCounts_.begin();
  for(; stpItr != stepCounts_.end(); ++stpItr) {
    // for each wordPairIds in stepCounts_  get tTable count
    LpPair<COUNT, PROB>* tc = getPtr(stpItr->first.first, stpItr->first.second);
    massert(tc != 0);
    //if(tc->count != 0)
    if(tc->count > PROB_SMOOTH)  // get interpolated count
      tc->count = ((OneMinusMu_k * tc->count) + (mu_k * stpItr->second));
...
void tmodel<COUNT, PROB>::normalizeTable(const vcbList& srcl, const vcbList& trgl, 
  const set<WordIndex>* srcHits, bool clearCounts)
{
  //vector< vector<pair<unsigned int,CPPair> >* > lexmat;
  //cerr << "Total lexmat size = " << lexmat.size() << endl;
  //const Vector<WordEntry>& srclist(srcl.getVocabList()); 
  //const Vector<WordEntry>& trglist(trgl.getVocabList());
  for(unsigned int i=0;i<lexmat.size();++i) // i = iterate through all source words
    {
      // only bother with source words that were "touched" during this step
      if(step_k != 0 && (srcHits->find(i) == srcHits->end())) continue;
      double c=0.0;
      if(lexmat[i]) 
	{
	  unsigned int lSize=lexmat[i]->size(); //lsize = # of possible translations for src_i 
	  for(unsigned int j=0;j<lSize;++j)  // j = a translation of src_i
          {
	    c+=(*lexmat[i])[j].second.count;
          }
          //cerr << "total count for '" << srclist[i].word << "' = " << c << endl; 
	  for(unsigned int j=0;j<lSize;++j)
		...
		(*lexmat[i])[j].second.prob=1.0/(lSize);
	      ...
	      if((step_k == 0) && clearCounts) (*lexmat[i])[j].second.count=0; // counts aren't cleared for OEM
	    }
	}
    }
}
            //cerr << "count for pair (" << srclist[i].word << "," << trglist[(*lexmat[i])[j].first].word << ") = " 
            //  << (*lexmat[i])[j].second.count << endl;
                //cerr << "prob for pair (" << srclist[i].word << "," << trglist[(*lexmat[i])[j].first].word << ") = " 
                  //<<  (*lexmat[i])[j].second.count << "/" << c << " = " << (*lexmat[i])[j].second.prob << endl;
template <class COUNT, class PROB>
...
//implemented 10/19/2009 ADL (copied from other version below)
template <class COUNT, class PROB>
